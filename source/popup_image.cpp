...
static void draw_boxed_text(const Font& font, const std::string& text, Vector2 text_pos, int textsz, Rectangle box, float spacing, Color color)
{
    int words_count;
...
void PopupImage::draw(const Font& font, const GameDrawBuffer& draw_buf) const
{
    assert(tex);
    ...
    if (state == PopupImageState::Shown)
    {
        ...
        DrawTextEx(font, "Mission 1: ", text_pos, 20, spacing, WHITE);
        ...
        spacing, BLACK);
        draw_boxed_text(font, body, body_pos,
        10,
        {
